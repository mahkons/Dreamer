...
class CouplingLayer(ConditionalFlow):
    def __init__(self, input_dim, condition_dim, hidden_shape, num_hidden, mask):
        ...
        modules_scale = [nn.Linear(input_dim + condition_dim, hidden_shape), nn.ReLU()] \
            + sum([[nn.Linear(hidden_shape, hidden_shape), nn.ReLU()] for _ in range(num_hidden)], []) \
            + [nn.Linear(hidden_shape, input_dim)]

        modules_translate = [nn.Linear(input_dim + condition_dim, hidden_shape), nn.ReLU()] \
            + sum([[nn.Linear(hidden_shape, hidden_shape), nn.ReLU()] for _ in range(num_hidden)], []) \
            + [nn.Linear(hidden_shape, input_dim)]
