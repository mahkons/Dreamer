...
class WorldModel(nn.Module):
    def __init__(self, state_dim, action_dim, device):
        ...
        log().add_plot("maf_first_bn_stats", ["running_mean_0", "running_var_0"])
    ...
    def optimize(self, obs, action, reward, discount):
        ...
        log().add_plot_point("maf_first_bn_stats", [
            self.flow_model.model.flow_modules[1].m[0].item(), # a bit of a hack
            self.flow_model.model.flow_modules[1].s[0].item()
        ])
    ...
    def get_embed(self, obs):
        ...
        if WITH_CONTRASTIVE:
            ...
            if self.training: # random in [0.9, 1.1); maybe better uniform noise?
                embed = embed * (torch.rand(embed.shape[0], embed.shape[1], 1, device=self.device) * 0.2 + 0.9) 
